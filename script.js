      ...
      senses: [
        {
          ...
          translation: "ном",
          examples: [
            { en: "I love reading a good book.", mn: "Би сайн ном унших дуртай." },
            { en: "She wrote a book about history.", mn: "Тэр түүхийн талаар ном бичсэн." }
          ],
          derivatives: [
            { word: "bookmark", translation: "номын хавчуурга" },
            { word: "bookshelf", translation: "номын тавиур" }
          ],
...
function renderEntry(lemma, entry, direction) {
  ...
  if (entry.senses && entry.senses.length > 1) {
    sensesHtml = entry.senses.map((sense, index) => {
      ...
      let senseTagsHtml = '';
      if (senseData.pos) {
        senseTagsHtml += `<button class="pos" onclick="showFilterList('pos', '${senseData.pos}')">${senseData.pos}</button>`;
      }
      if (senseData.topic) {
        senseTagsHtml += `<button class="topic-tag" onclick="showFilterList('topic', '${senseData.topic}')">${senseData.topic}</button>`;
      }
  ...
  let posToShow = entry.pos || (entry.senses ? entry.senses[0].pos : '');
  let topicToShow = entry.topic || '';
  let cefrToShow = entry.cefr || '';

  let tagsHtml = '';
  if (posToShow) {
    tagsHtml += `<button class="pos" onclick="showFilterList('pos', '${posToShow}')">${posToShow}</button>`;
  }
  if (topicToShow) {
    tagsHtml += `<button class="topic-tag" onclick="showFilterList('topic', '${topicToShow}')">${topicToShow}</button>`;
  }
  if (isHeadwordMongolian && cefrToShow) {
    tagsHtml += `<button class="level-tag" onclick="showFilterList('cefr', '${cefrToShow}')">${cefrToShow.toUpperCase()}</button>`;
  }
